...
const update = (req, res, doc) => {
  ...
  savePromise.then(() => res.json({ name: updateDog.name,
...
const setName2 = (req, res) => {
  ...
  savePromise.then(() => {
    ...
    lastAdded2 = newDog;
    // return success
    res.json({ name: lastAdded2.name, breed: lastAdded2.breed, age: lastAdded2.age });
  });
...
const updateLast2 = (req, res) => {
  ...
  // send back the name as a success for now
  savePromise.then(() => res.json({ name: lastAdded.name,
